...
@pytest.mark.asyncio
async def test_completed_llm_specialist_falls_back_when_no_evidence_available():
    async def deterministic_runner(**kwargs):
        return {
            "agent_name": "property_search",
            "status": "no_evidence",
            "content": "Chua co bang chung listing phu hop.",
            "evidence_ids_used": [],
            "warnings": [],
        }

    async def ungrounded_json(prompt: str, *, timeout_seconds=None):
        return {
            "agent_name": "property_search",
            "status": "completed",
            "content": "unsupported answer without retrieved evidence",
            "claims": [],
            "evidence_ids_used": [],
            "confidence": "medium",
            "warnings": [],
    ...
    result = await run_llm_or_deterministic_specialist(
        ...
        preferences={},
        readiness={},
        generate_json=ungrounded_json,
    )

    assert result["status"] == "no_evidence"
    assert result["content"] == "Chua co bang chung listing phu hop."
    assert "llm_specialist_missing_evidence" in result["warnings"]


@pytest.mark.asyncio
async def test_disclaimer_only_claims_fall_back_for_source_backed_content():
    async def deterministic_runner(**kwargs):
        return {
            "agent_name": "property_search",
            "status": "completed",
    ...
    async def disclaimer_only_json(prompt: str, *, timeout_seconds=None):
        return {
            "agent_name": "property_search",
            "status": "completed",
            "content": "unsupported answer with only disclaimer",
            "claims": [
                {"type": "disclaimer", "text": "Can tu tham dinh them."},
            ],
            "evidence_ids_used": ["ev1"],
            "confidence": "medium",
            "warnings": [],
    ...
    result = await run_llm_or_deterministic_specialist(
        ...
        preferences={},
        readiness={},
        generate_json=disclaimer_only_json,
    )

    assert result["content"] == "deterministic grounded answer"
    assert "llm_specialist_missing_evidence_claims" in result["warnings"]
